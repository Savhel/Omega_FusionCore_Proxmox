...
```
1. MigrationPolicy détecte nœud surchargé (RAM > 85%)
2. Recommandation : migrer VM X vers nœud Y
...
7. cleanup_after_migration() : supprime pages store, libère slots vCPU
```
...
Expose l'état du nœud : RAM, pages stockées, VMs locales avec leur `avg_cpu_pct`, `throttle_ratio`, `remote_pages`. Interrogé par le controller Python.

### Control API (port 9300, HTTP)
Canal de contrôle local : éviction, quotas par VM, vCPU, migrations live/cold, métriques Prometheus (`/control/metrics`).
...
### vCPU Scheduler
Allocation élastique : 3 vCPU par pCPU, jusqu'à 3 VMs par slot. Hotplug si usage > 80%. Migration recommandée si steal time > 10% (nœud saturé). Données alimentées par `update_from_cgroup()` depuis les cgroups v2.
...
### GPU Multiplexer
Partage d'un GPU physique entre plusieurs VMs. Budget VRAM configurable par VM.

## Composants du controller (Python)

### MigrationPolicy
Vue cluster-wide. Collecte `GET /control/status` sur les 3 nœuds. Décide : quoi migrer, vers où, live ou cold. Règles : RAM > 95% → cold forcée, RAM 85–95% → live, VM idle > 60s → cold, VM throttlée → live.
