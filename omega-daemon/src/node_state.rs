...
impl NodeState {
    pub fn new(
        ...
        Self {
            node_id, store_addr, api_addr,
            store, metrics, vm_tracker,
    ...
    pub fn snapshot(&self) -> NodeStatus {
        ...
        let vcpu_stats: std::collections::HashMap<u32, (f64, f64)> = {
            self.vcpu_scheduler.vm_snapshot()
                .into_iter()
                .map(|s| {
        ...
        let vm_entries: Vec<VmStatusEntry> = vms.iter().map(|vm| {
            let (avg_cpu_pct, throttle_ratio) = vcpu_stats
                .get(&vm.vmid)
                .copied()
                .unwrap_or((0.0, 0.0));
            VmStatusEntry {
    ...
    pub fn pages_per_vm(&self) -> Vec<(u32, u64)> {
        ...
        let vms = self.vm_tracker.local_vms_snapshot();
        let mut result: Vec<(u32, u64)> = vms.iter()
