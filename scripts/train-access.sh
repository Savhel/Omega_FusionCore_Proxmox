#!/usr/bin/env bash
# train-access.sh — Ouvre (ou ferme) l'accès d'une VM Omega isolée à l'environnement d'entraînement GPU (LXC sur nœud GPU).
#
...
# l'hôte de l_env d_entrainement GPU (Jupyter 8888 + SSH 22), rien d'autre du LAN.
# La VM reste donc coupée d'internet/du reste du LAN. Réversible.
#
# Usage :
#   scripts/train-access.sh --vmid 3000 --enable
#   scripts/train-access.sh --all --enable            # tout le réseau OMEGA
#   scripts/train-access.sh --ip 10.50.30.101 --disable
#   scripts/train-access.sh --list
#
# Options :
...
#   --list          lister les accès entraînement ouverts.
#   --gateway IP    Hôte de l_env entrainement GPU (défaut 192.168.123.60).
#   --gw-port P     Port Jupyter (défaut 8888).
...
GATEWAY_IP="${OMEGA_TRAIN_HOST_IP:-192.168.123.60}"
GW_PORT="${OMEGA_TRAIN_PORT:-8888}"
...
write_config('omega: train-access enable ${VM_IP} -> ${GATEWAY_IP}');
filter_configure();
echo 'OK';
PHPEOF
)
...
write_config('omega: train-access disable ${VM_IP}');
filter_configure();
echo 'OK';
PHPEOF
)
...
global \\\$config;
foreach (\\\$config['filter']['rule'] ?? [] as \\\$r) {
    if (strpos(\\\$r['descr'] ?? '', 'omega-train-') === 0) {
...
case "$ACTION" in
    enable)
        ...
        if [[ "$result" == "EXISTS" ]]; then
            info "Accès entraînement déjà ouvert pour ${VM_IP} → ${GATEWAY_IP}"
        else
            ok "Accès entraînement ouvert : ${SRC_CIDR} → ${GATEWAY_IP}:${GW_PORT} (gateway uniquement)"
        fi
        ;;
    disable)
        [[ -n "$SRC_CIDR" ]] || fail "--vmid, --ip ou --all requis pour --disable"
        pfsense_php "$DISABLE_PHP" >/dev/null
        ok "Accès entraînement retiré pour ${VM_IP}"
        ;;
    list)
        info "Accès entraînement ouverts :"
