...
info "VM $VMID : source=$node_init → cible=$TARGET_NODE"
...
info "Lancement migration live : qm migrate $VMID $TARGET_NODE --online"
t_migrate=$SECONDS
qm migrate "$VMID" "$TARGET_NODE" --online 2>&1 | tee /tmp/omega-m5-migrate.log
migrate_status=$?
...
vm_running=$(qm status "$VMID" 2>/dev/null | grep -c "running" || echo 0)
[[ "$vm_running" -gt 0 ]] || fail "VM $VMID n'est plus en cours d'exécution après migration"
